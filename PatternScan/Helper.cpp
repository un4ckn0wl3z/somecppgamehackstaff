    ...
    if (Module32First(hSnapshot, &ModuleEntry32))
    {
        do {
            if (_tcscmp(ModuleEntry32.szModule, lpszModuleName) == 0)
            {
                ...
                this->pBaseAddress = (DWORD)ModuleEntry32.hModule;
                this->pSize = (DWORD)ModuleEntry32.modBaseSize;
                break;
            }
...
uintptr_t Helper::GetDynamicAddress(uintptr_t baseAddress, std::vector<DWORD> offsets) {
...
uintptr_t Helper::GetAddressFromSignature(std::vector<int> signature) {
    ...
    for (int i = 0; i < this->pSize; i++) {
        for (uintptr_t j = 0; j < signature.size(); j++) {
            ...
            if (signature[j] == memBuffer[i + j] && j > 0)
                std::cout << "Dump byte: " << std::hex << int(signature[j]) << std::hex << int(memBuffer[i + j]) << j << std::endl;
