...
struct _Player {
	void* pThis = NULL;
	bool bghost = false;
	DWORD ghostoffset = 0x0000067F;
	DWORD xoffset = 0x28;
	DWORD yoffset = 0x2C;
	DWORD health = 0x3D8;
	DWORD xteleport = 0;
	DWORD yteleport = 0;
	DWORD xflyoffset = 0x34;
...
DWORD GetPlayerBase() {
	...
	std::cout << "GetCurrentPlayerFunctionBaseAddress: " << std::hex << Entry << std::endl;

	DWORD eax = *(DWORD*)(*(DWORD*)(Entry + 0x01));
	DWORD edx = *(DWORD*)(*(DWORD*)(Entry + 0x07));
	DWORD basePlayerAddress = *(DWORD*)(eax + edx * 4 + 0x08);
	std::cout << "basePlayerAddress: " << std::hex << basePlayerAddress << std::endl;
...
DWORD WINAPI Menue() {
	...
	while (1) {
		...
		if (GetAsyncKeyState(VK_NUMPAD4)) {
			...
			if (Player.pThis == NULL)
				Player.pThis = (void*)GetPlayerBase();
			*(DWORD*)((DWORD)Player.pThis + Player.health) = 100;
		}

		if (GetAsyncKeyState(VK_NUMPAD5)) {
			std::cout << "Starting fly function" << std::endl;
			if (Player.pThis == NULL)
				Player.pThis = (void*)GetPlayerBase();
			*(float*)((DWORD)Player.pThis + Player.xflyoffset) = *(float*)((DWORD)Player.pThis + Player.xflyoffset) -= 3.00;
		}
	}
	fclose(fp);
	FreeConsole();
